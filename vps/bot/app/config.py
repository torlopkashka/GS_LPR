...
class BotConfig:
    ...
    # Общий секрет для запросов с ПК у ворот (VPS_TOKEN в local/.env должен совпадать)
    site_token: str = ""
    # Объект считается на связи, если присылал данные не позже N секунд назад
    site_online_timeout: float = 45
    # Сообщить в чат, если объект не на связи дольше N секунд (0 — не сообщать)
    site_offline_alert_after: float = 180
    # Правка списка, которую объект так и не забрал за N секунд, отменяется
    op_ttl: float = 7 * 86400
...
def load_config() -> BotConfig:
    ...
    cfg = BotConfig(
        ...
        site_online_timeout=float(e("SITE_ONLINE_TIMEOUT", "45")),
        site_offline_alert_after=float(e("SITE_OFFLINE_ALERT_AFTER", "180")),
        data_dir=Path(e("DATA_DIR", "data")),
    )
    if not cfg.site_token:
        raise RuntimeError("Не задан SITE_TOKEN (см. vps/.env.example)")
